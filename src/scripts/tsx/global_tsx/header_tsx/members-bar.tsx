...
const MembersBar :React.FC<MembersBarProps> = ({setModalVisibility}) => {
    ...
    return (
        <div className="members-bar_wrapper">
            ...
            <button className="members-bar_login-button" style={{display: isMobile && !context.loggedIn? "block" : mLoginDisplay, top: isMobile &&context.loggedIn ? "75px" : "25px"}} onClick={setUserStatus}>{context.loggedIn ? "Log Out" : "Log In"}</button>
        </div>
    )
}
